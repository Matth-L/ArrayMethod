...
let agePlusGrandQue = (tab, ageDemande) => { // on rentre le tableau et l'age 
    let newT = tab.filter(({ age }) => { return (age > ageDemande) }); // filter va sur tous les et effectue un test pour filter le tab
    return newT;
};

//fonction permettant de filtrer le sexe , pas de fonction fléché car fonctionne mal avec this
function sexChoose({ sex }) {
    return this.has(sex); // this has obj.sex va en realité regarder dans le parametre juste a coté 
